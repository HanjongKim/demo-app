...
public class RabbitMQConfig {
	...
	private String addresess;//="127.0.0.1:25675,127.0.0.1:25673,127.0.0.1:25674";
	//private String addresess="127.0.0.1:25675";
	
	@Value("${vcap.service.rabbitmq.username}")
	private String username;//="rmqadmin";
	
	@Value("${vcap.service.rabbitmq.password}")
	private String password;//="dahR5lioxeiC9oon";
    ...
    List<Queue> queues(){
		...
		args.put("x-message-ttl", 100000); 							// TTL(timeout) 10s
		args.put("x-max-length", 10);								// Message count
		args.put("x-max-length-bytes", 1000000);					// Message size 1M
		
		queues.add(new Queue(DEAD_LETTER_QUEUE, true,false, false));
		queues.add(new Queue(DEMO_IMAGE_QUEUE, true, false, false, args));
		queues.add(new Queue(DEMO_IMAGE_QUEUE_RESULT, true, false, false, args));
		queues.add(new Queue(DEMO_IMAGE_QUEUE_INDEX+index, true, false, false, args));
		queues.add(new Queue(HYSTRIX_QUEUE, true, false, false, args));
    ...
    List<TopicExchange> exchange(){
    	...
    	topicExchange.add(new TopicExchange(EXCHANGE, true, false));
    	topicExchange.add(new TopicExchange("springCloudHystrixStream", true, false));
