...
import com.skb.demo.config.RabbitMQConfig;
...
public class RabbitMQServiceController{
	...
	private static final Logger logger = LoggerFactory.getLogger(RabbitMQServiceController.class.getName());
    ...
    public void processMessage1(Map<String, Object> message) {
	    ...
	    for (MessageListenerContainer listenerContainer :messageListenerContainers) {
	        System.out.println("asdfasdf" + listenerContainer);
	        listenerContainer.stop();
	    }

	    System.out.println(" [x] Received '" + message.get("TID") + "'");
	    try {
			Thread.sleep(10000);
		} catch (Exception e) {
